function [x,r,g,info] = spgLR(A, b, tau, sigma, x, options, params)
...
% %        g2Norm  = undist(options.dual_norm(g2,weights,params));
    ...
    gNorm = rNorm; % Hassan: initialize gNorm to dummy value
...
%           tau      = max(0,tau + (rNorm * aError1) / (gNorm)); % deleted rNorm from numerator. In this algorithm, ony gNorm with contain derivative information. 
          nNewton  = nNewton + 1;
...
%     end
    ...
    try
       ...
       [f,x,r,nLine,stepG,lnErr, localProdA] = ...
           spgLineCurvy(x,gStep*g,max(lastFv),funForward, funPenalty, b,@project,tau, params, lineSrchIt);
...
function [fNew,xNew,rNew,iter,step,err, nProd] = ...
    spgLineCurvy(x,g,fMax,funForward, funPenalty, b,project,tau, params, lineSrchIt)
