...
for j = 1:5
    ...
    Dinit            = F(m0, Q, model);
    Dinit            = reshape(Dinit, nrec, nsrc, length(model.freq));
    model.W          = W;
    % interpolate full data without PDE constraint
    [Dfull,SNR]      = spg_interp_joint(Dtrue,model,Mask,rank(j));  % <****************why is this calling a mask?
    SNR
    options.maxIter  =  15;
...
% SNR
% [Dfull1,SNR1]    = spg_interp_joint( Dtrue,Dinit,model,Mask,rank(j));
%
